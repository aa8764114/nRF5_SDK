...
#define SS2_BIKE_PRODUCT_MODEL          (3)
    ...
    case OP_ITEM_READ_IR_ER:
        if (session_status == SESSION_CONNECTED && session->rx_buffer.used == 2)
        {
            ...
            }
            {
                ...
                NRF_LOG_INFO("[%s] sent sesame_IR&ER", __func__);
            }
        }
        else
        {
...
void ble_ssm2_advertising_init(void)
{
    ...
    ssm2.adv.header.product_model = SS2_BIKE_PRODUCT_MODEL;
    if (ssm2.registered)
    {
        ssm2.adv.header.is_registered = 1;
...
{
    uint8_t ir_er_32byte[32] = {0x10,0x11,0x012,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f,
                                0xa0,0xa1,0x0a2,0xa3,0xa4,0xa5,0xa6,0xa7,0xa8,0xa9,0xaa,0xab,0xac,0xad,0xae,0xaf};
