...
static void services_init(void)
{
    ...
    //link total應該會跟m_qwr的真列大小一樣，但是他define的東西不同？
    for (uint32_t i = 0; i < LINK_TOTAL; i++)
    {
        err_code = nrf_ble_qwr_init(&m_qwr[i], &qwr_init); //初始化藍牙GATT佇列
        APP_ERROR_CHECK(err_code);
    }
...
static void on_connected(const ble_gap_evt_t * const p_gap_evt)
{
    ...
    // 看來開監聽後就可以得到每個裝置的conn_handle，m_qwr是另外存進陣列
    for (uint32_t i = 0; i < NRF_SDH_BLE_PERIPHERAL_LINK_COUNT; i++)
    {
        if (m_qwr[i].conn_handle == BLE_CONN_HANDLE_INVALID)    //如果某個裝置的handle無效，指定為p_gap_evt->conn_handle
        {
    ...
    err_code = app_button_enable(); //開啟按鍵
    APP_ERROR_CHECK(err_code);
...
static void on_disconnected(ble_gap_evt_t const * const p_gap_evt)
{
    ...
    if (periph_link_cnt == 0)   //如果沒有裝置連線，就把有裝置連線的LED燈關閉，並且把按鍵關閉
    {
    ...
    if (periph_link_cnt == (NRF_SDH_BLE_PERIPHERAL_LINK_COUNT - 1)) //如果目前只有三個裝置連線，就開啟廣播
    {
