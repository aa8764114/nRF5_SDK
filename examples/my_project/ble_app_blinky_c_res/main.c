...
static void lbs_c_evt_handler(ble_lbs_c_t * p_lbs_c, ble_lbs_c_evt_t * p_lbs_c_evt)
{
    ...
    //p_lbs_c_evt->evt_type就是中心設備監聽收到的事件（雖然參數傳來傳去但就是這個）
    //這兩個事件是自己在ble_lbs_c_res.c寫進去的，所以其實可以自己定事件，在自己寫進結構裡，再透過監聽去做反應
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_CONNECTED:
        {
            ...
            err_code = ble_db_discovery_start(&m_db_disc, p_gap_evt->conn_handle);   //經過這個函數剛剛設定的函數就會觸發
            APP_ERROR_CHECK(err_code);
 ...
 */
//初始化lbd_c服務：
//先把資訊在lbs_c_init_obj填好，再透過ble_lbs_c_init把東西填到m_ble_lbs_c裡
static void lbs_c_init(void)
{
    ...
    lbs_c_init_obj.evt_handler   = lbs_c_evt_handler;   //收到事件時應該做什麼事（內涵監聽外圍設備發送的訊號，決定開關燈）
    lbs_c_init_obj.p_gatt_queue  = &m_ble_gatt_queue;   //gatt駐列
    lbs_c_init_obj.error_handler = lbs_error_handler;   //不知道
...
static void button_event_handler(uint8_t pin_no, uint8_t button_action)
{
    ...
    switch (pin_no)
    {
        ...
        case LEDBUTTON_BUTTON_PIN:
            ...
            if (err_code == NRF_SUCCESS)
            {
                NRF_LOG_INFO("[送] [%x]", button_action);
            }
            break;
